...
import numpy as np
from tkbcmodels import TComplEx
...
class AttentionLayer(nn.Module):
	...
	def forward(self, input1, input2):
		...
		attention_weights = torch.sigmoid(self.fc(combined_input))
        
		return attention_weights
class TA_CQA(nn.Module):
	def __init__(self, tkbc_model, args):
		...
		self.entity_time_embedding1 = nn.Embedding(num_entities + num_times + 1,
																self.tkbc_embedding_dim,
																padding_idx=num_entities + num_times)
		self.entity_time_embedding1.weight.data[:-1, :].copy_(full_embed_matrix)
        
		
		self.num_entities =num_entities
		#self.attention_layer = AttentionLayer(self.transformer_dim, self.nhead)
		self.attention_layer = AttentionLayer(self.transformer_dim)
		...
		self.linear3 = nn.Linear(self.tkbc_embedding_dim, self.tkbc_embedding_dim)

		self.linear4 = nn.Linear(self.tkbc_embedding_dim, self.tkbc_embedding_dim)
	...
	def score_time(self, head_embedding, tail_embedding, relation_embedding):
		...
		#timeall = self.entity_time_embedding.weight
		#time= timeall[self.num_entities:-1, :]
	...
	def forward(self, a):
		...
		attention_weights = self.attention_layer(masked_question_embedding, masked_entity_time_embedding)
		combined_embed = attention_weights * masked_question_embedding + (1 - attention_weights) * masked_entity_time_embedding
		...
		entity_time_embeddings = self.entity_time_embedding1.weight.data[:-1, :]
		interaction_embeddings = self.linear4(entity_time_embeddings)

		
		scores_time1 = self.score_time(head_embedding, tail_embedding, relation_embedding1)
		scores_time2 = torch.matmul(relation_embedding1, interaction_embeddings[self.num_entities:, :].T)
