...
from TA_CQA_model import TA_CQA, AttentionLayer
from qa_datasets import QA_Dataset, QA_Dataset_TA_CQA, QA_Dataset_Baseline
...
parser.add_argument(
    '--batch_size', default=180, type=int,
    help="Batch size."
)
...
if args.model == 'bert' or args.model == 'roberta':
    ...
    #valid_dataset = QA_Dataset_baseline(split=args.eval_split, dataset_name=args.dataset_name)
    test_dataset = QA_Dataset_Baseline(split=test, dataset_name=args.dataset_name)
elif args.model in ['TA_CQA', 'entityqr', 'cronkgqa']:  #supervised models
    qa_model = TA_CQA(tkbc_model, args)
    if args.mode == 'train':
        dataset = QA_Dataset_TA_CQA(split=train_split, dataset_name=args.dataset_name, args=args)
    #valid_dataset = QA_Dataset_TempoQR(split=args.eval_split, dataset_name=args.dataset_name, args=args)
    test_dataset = QA_Dataset_TA_CQA(split=test, dataset_name=args.dataset_name, args=args)
else:
